...
def _supported(row):
    ...
    if 'Y' in (row['DMR'], row['P25_PHASE_1'], row['P25_PHASE_2'],
               # row['FUSION'],  # Fusion also operates Analog
               row['NXDN_DIGITAL'], row['ATV'], row['DATV']):
...
def lambda_handler(event=None, context=None):
    ...
    client.put_object(Bucket=dst_bucket, Key=dst_key,
                      Body=string_obj.getvalue(), ContentType='text/csv',
